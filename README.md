...
> 
> **⚙️ 显存奇迹：仅需 8G 显存，即可在消费级显卡（如 RTX 2080 Ti、RTX 3070、RTX 4060 等）上强行流畅跑起顶配 0.25° 全球高分辨率物理模型，榨出 Google 官方 TPU v4 级别的巅峰推理吞吐量！**
> 
...
| 资源类别 | 适用场景与规格说明 | 下载链接 |
| :--- | :--- | :--- |
| **0.25度测试数据** | 由于数据过大，没有包含在测试数据里，请到谷歌云下载 | [📥 下载测试数据包](https://storage.cloud.google.com/dm_graphcast/graphcast/dataset/source-era5_date-2022-01-01_res-0.25_levels-37_steps-01.nc)
| **通用测试数据** | 1.0°/0.25° 基础测试输入、均值与标准差归一化参数（必下） | [📥 下载测试数据包](https://github.com/VectorElectron/graphcast-lite/releases/download/weights/testdata_and_normvector.zip) |
| **Lite 权重** (PyTorch端) | 1.0° 基础模型，用于 PyTorch / NumPy / CuPy 快速验证与微调 | [📥 下载 1.0° 权重包](https://github.com/VectorElectron/graphcast-lite/releases/download/weights/lite_5mesh_13level_1deg.zip) |
| **Lite 权重** (PyTorch端) | 0.25° 高清模型，用于 PyTorch 原生高精度自回归研究与微调 | [📥 下载 0.25° 权重包](https://github.com/VectorElectron/graphcast-lite/releases/download/weights/lite_6mesh_37level_0.25deg.zip) |
| **Turbo 权重** (TensorRT端) | 1.0° 编译源包（含特定 ONNX 与 configs），用于 TRT 极速推理 | [📥 下载 1.0° 编译包](https://github.com/VectorElectron/graphcast-lite/releases/download/weights/turbo_5mesh_13level_1deg.zip) |
| **Turbo 权重** (TensorRT端) | 0.25° 编译源包（含特定 ONNX 与 configs），用于全球极速生产推理 | [📥 下载 0.25° 编译包](https://github.com/VectorElectron/graphcast-lite/releases/download/weights/turbo_6mesh_37level_0.25deg.zip) |


> 📌 **路径对齐要求**：测试数据请放置在 `weights/testdata_and_normvector/` 目录下；模型权重请放置在如 `weights/lite_5mesh_13level_1deg/...` 或 `weights/turbo_6mesh_37level_0.25deg/...` 的对应目录下。
...
```bash
# 在 Layer 1 & 2 的基础上，额外安装 TensorRT 推理加速引擎，注意v11的编译参数有较大调整，这里用v10
...
```bash
# 编译并开启 FP16（以 1deg 5mesh 13level 精度配置为例） --fp16  --bf16  --noTF32 等flag
...
| :--- | :---: | :---: | :---: | :---: |
| **RTX 2080 Ti** | Turbo TRT (FP32) | 115 s | ~ 7.8 GB | 恒定无碎片 |
| **RTX 2080 Ti** | Turbo TRT (FP16) | 40 s | ~ 6.0 GB | 恒定无碎片 |
| **RTX 4090** | Turbo TRT (TF32) | 29 s | ~ 7.8 GB | 恒定无碎片 |
| **RTX 4090** | Turbo TRT (BF16) | 15 s | ~ 6.0 GB | 恒定无碎片 |
...
本项目虽然完全开源，但需要说明的是，**Turbo 极致加速引擎中的 ONNX 子图分解与重新编排，部分是torch代码导出，又使用了部分第三方工具链，并结合了极其复杂的手工计算图物理重组与算子融合技术**。这导致子图onnx文件无法简单的一键生成。

如果您正在面临以下场景，欢迎与我们联系，开展深入的**学术探讨**或**商业合作**：

* **自有微调权重（Fine-tuned Weights）**：您已基于 GraphCast 官方架构微调出了特定区域（如中国局部高分辨率）或特定要素的私有权重，希望能无缝嵌入到 Turbo 引擎中，在低配硬件上压榨出极致的推理吞吐量。
...
* **除Tensorrt之外的环境中高效部署**: 例如非Nvidia显卡，AMD卡，国产显卡，甚至Rockchip等边缘设备部署。目前**Turbo**版本基于Tensorrt开发，但相同的技术，应用于非Nvidia设备时完全有可行性的。
